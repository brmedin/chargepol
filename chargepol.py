...
def write_output(filename_output, pos_time, pos_zmin, pos_zwid, pos_flax, pos_flay, neg_time, neg_zmin, neg_zwid, neg_flax, neg_flay):
    ...
    e.write("#charge (pos/neg), time (UT sec), bottom height of layer (km), depth of layer (km), x distance from LMA center (km), y distance from LMA center (km)\n")
    e.write("charge,time,zmin,zwidth,x,y\n")
    for f in np.arange(0,len(pos_time)):
        e.write("pos,%f,%f,%f,%f,%f,%f,%f\n" % (pos_time[f], pos_zmin[f], pos_zwid[f], pos_flax[f], pos_flay[f]))

    for g in np.arange(0,len(neg_time)):
        e.write("neg,%f,%f,%f,%f,%f,%f,%f\n" % (neg_time[g], neg_zmin[g], neg_zwid[g], neg_flax[g], neg_flay[g]))
